        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "   <thead ng-hide=\"tree_rows.length == 0\">\n" +
                    "     <tr>\n" +
                    ...
                    "       <th class=\"visible-lg\" ng-repeat=\"col in colDefinitions\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
                    "     </tr>\n" +
                    "   </thead>\n" +
                    "   <tbody>\n" +
                    ...
                    "       <td ng-class=\"row.branch.isDir && row.branch.hasChildren ? 'cursor-pointer':''\" ng-click=\"on_open_click(row.branch, row)\"\n><a ng-click=\"user_clicks_branch(row.branch)\"><i ng-class=\"!row.branch.isDir ? row.tree_icon : (row.branch.hasChildren ? row.tree_icon : '') \"\n" +
                    "              class=\"indented\"></i></a><span ng-if=\"expandingProperty.cellTemplate\" class=\"indented\" " +
                    "              ng-click=\"on_user_click(row.branch)\" compile=\"expandingProperty.cellTemplate\"></span>" +
                    "              <span  ng-if=\"!expandingProperty.cellTemplate\" class=\"indented\" ng-click=\"on_user_click(row.branch)\">\n" +
                    "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span>\n" +
                    "       </td>\n" +
                    "       <td ng-class=\"'visible-lg' + (row.branch.isDir && row.branch.hasChildren ? ' cursor-pointer':'')\" ng-click=\"on_open_click(row.branch, row)\"\n ng-repeat=\"col in colDefinitions\">\n" +
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        var getExpandingProperty = function getExpandingProperty() {
                            if (attrs.expandOn) {
                                scope.expandOn.displayName = scope.colsDisplayName.title;
                        ...
                        if (!attrs.colDefs) {
                            ...
                            scope.colDefs[0].displayName = scope.colsDisplayName.name;
                        ...
                        scope.on_open_click = function (branch, row) {
                            if(scope.onOpen){
                                scope.onOpen({ branch: branch, row: row });
                            }
                            row.tree_icon = "naviair-fonts-spin3 animate-spin";
                        };
                        ...
                        on_treeData_change = function () {
                            ...
                            add_branch_to_list = function (level, branch, visible) {
                                ...
                                if ((!branch.children || branch.children.length === 0) && !branch.isDir) {
